...
class ModelBinding(ContractModel):
    ...
    @classmethod
    def validate_extra_headers(cls, value: dict[str, str]) -> dict[str, str]:
        for name, header_value in value.items():
            if not name or len(name) > 100 or len(header_value) > 4_000:
                raise ValueError("model extra header is invalid")
            if "\r" in name or "\n" in name or "\r" in header_value or "\n" in header_value:
                raise ValueError("model extra header contains a newline")
        return value
...
class ProjectFileSnapshot(ContractModel):
    ...
    def validate_project_file_path(cls, value: str) -> str:
        normalized = value.replace("\\", "/")
        parts = normalized.split("/")
        if len(parts) != 3 or parts[0] != "project-files" or any(part in {"", ".", ".."} for part in parts):
...
class Attachment(ContractModel):
    ...
    data: str = Field(min_length=1, max_length=16_000_000)
...
class ExecutionManifest(ContractModel):
    runtime_type: Literal["agentscope"] = "agentscope"
