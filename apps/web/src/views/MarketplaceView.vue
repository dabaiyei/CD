...
async function openPublish(source?: PublicationSource): Promise<void> {
  ...
  if (!sources.value.length) {
    if (kind.value === 'template') {
      ...
      toast.show(kind.value === 'skill' ? '还没有可发布的个人 Skill' : '还没有可发布的个人素材', { tone: 'info' })
    }
    return
  }
    ...
    <BaseDialog :open="dialog === 'publish'" :title="config.action" description="选择一个自己的资源并发布当前版本快照" wide @update:open="!$event && !publishing && (dialog = null)">
      <form id="marketplace-publish-form" class="marketplace-publish" @submit.prevent="publishSelected">
        ...
        <div v-else class="marketplace-source-grid">
          <button v-for="source in sources" :key="source.id" type="button" :aria-pressed="selectedSourceId === source.id" @click="selectSource(source)"><span><img v-if="'media_url' in source && source.media_url" :src="source.media_url" alt="" /><component :is="sourceAssetMeta(source)?.icon ?? (kind === 'skill' ? Sparkles : FileText)" v-else :size="19" /></span><div><strong>{{ source.name }}</strong><small v-if="'scope' in source">{{ source.scope === 'project' ? '项目素材' : '全局素材' }}</small><p>{{ sourceDescription(source) }}</p></div><Check :size="17" /></button>
        </div>
...
.marketplace-source-grid { display: grid; max-height: 390px; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; overflow-y: auto; padding: 1px; }.marketplace-source-grid>button { display: grid; min-height: 88px; grid-template-columns: 48px minmax(0, 1fr) 18px; align-items: center; gap: 10px; padding: 9px; border: 0; border-radius: 8px; color: var(--ink); background: var(--surface-strong); box-shadow: inset 0 0 0 1px var(--line); text-align: left; transition-property: box-shadow, background-color, scale; transition-duration: var(--duration-fast); }.marketplace-source-grid>button:active { scale: .96; }.marketplace-source-grid>button[aria-pressed='true'] { background: var(--brand-soft); box-shadow: inset 0 0 0 1px var(--brand); }.marketplace-source-grid>button>span { display: inline-flex; width: 48px; height: 48px; align-items: center; justify-content: center; overflow: hidden; border-radius: 6px; color: var(--brand); background: var(--surface); }.marketplace-source-grid img { width: 100%; height: 100%; object-fit: cover; }.marketplace-source-grid strong, .marketplace-source-grid small, .marketplace-source-grid p { display: block; overflow: hidden; }.marketplace-source-grid strong { font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.marketplace-source-grid small { width: max-content; margin-top: 3px; padding: 2px 5px; border-radius: 4px; color: var(--brand-strong); background: var(--brand-soft); font-size: 8px; }.marketplace-source-grid p { display: -webkit-box; margin: 3px 0 0; color: var(--ink-tertiary); font-size: 9px; line-height: 1.4; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }.marketplace-source-grid>button>svg { opacity: 0; scale: .25; filter: blur(4px); color: var(--brand); transition-property: opacity, scale, filter; transition-duration: 300ms; transition-timing-function: cubic-bezier(.2, 0, 0, 1); }.marketplace-source-grid>button[aria-pressed='true']>svg { opacity: 1; scale: 1; filter: blur(0); }
