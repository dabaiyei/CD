...
async def execute(task_id, runtime_factory):
    ...
    for offset in range(0, len(manifest), 3):
        ...
        prompt = (
            ...
            "一致性检查。角色/角色衍生必须写入人物设定图规则：一张完整画面里按左一右三排布同一人物四视图，"
            "左侧是上半身正面特写，右侧并列全身正面、全身侧面、全身背面，服装发型头饰完全一致、完整站立全身入镜、"
            "三视图比例协调且共用脚底基线；四视图必须画在同一张连续画面里，不能是几张照片拼接或分格拼版；"
            "统一头身、服装、光线且不能变成四个不同人物，背景用干净的中性柔光棚拍背景；其它资产不套人物四视图。"
