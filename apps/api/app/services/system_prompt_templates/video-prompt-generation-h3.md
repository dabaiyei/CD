...
- 普通交锋在英文正文明确高速实时连攻（rapid real-time chained attacks and counters），保留连续变招与攻防接触。一个主要动作链不是一次出手；一秒内可有多次短距离接触，禁止误读下述连续性要求为限制出招频率。普通挥砍与格挡不默认慢放；只有大招蓄势、显形或关键命中展示才短暂放慢，然后明确回到实时速度。遵守分镜明确的静止与停顿。
...
- `integrated_multimodal_description` 或 `detailed_description` 必须写出镜内主要动作的连续路径、关键接触反馈与自然收势，不能只列起止姿势或每秒换一个动作。首帧/身份保留不等于锁死后续姿势；`fully_preserved` 不能与指定人物动作冲突。连续拍摄中不擅加跳切、姿势复位、延时或速度突变，不用晃动、模糊代替中间动作。
