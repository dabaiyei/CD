...
- 战斗镜头在生成视频提示词前会先生成独立首帧，明确该 0 秒瞬间的相机位置、画面左中右、身体朝向、视线目标和持物，避免用人物立绘充作开场导致主体不看对手。判定「参考图是否只是资产立绘」同时依据资产 ID 与精确 URL：资产重新生成后文件名改变但 ID 不变，只比对 URL 会把过期立绘误判为已设计首帧而跳过生成。
