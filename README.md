...
- 连续场景视频采用尾帧接力：新分镜通过 `continuity_group` 划分连续区间（保存在分镜版本内容中），同组后镜等待前镜完成、提取实际尾帧，再用它替换第一张图片参考。不同场景保持并行。旧分镜按共同场景资产或一致场景说明保守识别，明确跳时/转场不接力。
- 接力使用项目视频模型；若模型及供应商适配器均支持视频参考，附带上一镜末尾一秒的无声运动参考，否则采用首帧参考。依赖与参考文件持久保存，Worker 重启可恢复；上一镜失败时后镜不会空跑，重做上游后依赖它的旧视频自动失效。
...
管理员进入「创作手册 → AI 创建画风」，上传 1–4 张 JPG、PNG 或 WebP 参考图即可提交后台任务（单图最多 32MB，合计最多 64MB）。使用当前租户的默认文本模型分析图片，该模型必须支持视觉输入且平台已配置 API Key。
...
- 武指正文最多 4800 字符；当前阶段手册/个人技能检索片段合计最多 9000 字符，每文件最多1600字符。个人技能先根据名称及说明目录匹配，最多两项。仅打斗子任务使用此预算，不修改用户保存的完整手册。单次打斗编排输入超过32000字符时明确失败，不把裁剪后的动作冒充完整结果。
